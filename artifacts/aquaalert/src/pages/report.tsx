...
import { MapPin, Droplets, Send, CheckCircle, Leaf, ArrowLeft, Camera, X, Image } from "lucide-react";
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      img.onload = () => {
        ...
        const MAX_W = 900; const MAX_H = 700;
...
function SuccessScreen({ onReset }: { onReset: () => void }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { fire(60, { x: 0, y: 0.65 }); fire(120, { x: 1, y: 0.65 }); }, 150);
        ...
        <CardContent className="p-8 flex flex-col items-center text-center gap-5">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              Thank you for helping Mumbai conserve water. Your report is now live on the map.
            </p>
          </motion.div>
...
export default function Report() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      title: "", description: "", severity: "medium", ward: "",
      reporterName: localStorage.getItem("aquaalert_reporter_name") ?? "",
    },
  });

  const handleReset = () => { setIsSuccess(false); setStep(1); form.reset(); setPosition(null); setImageDataUrl(null); };
  ...
  const onSubmit = (data: FormValues) => {
    ...
    if (name) {
      localStorage.setItem("aquaalert_reporter_name", name);
      if (!localStorage.getItem("aquaalert_member_since")) {
        localStorage.setItem("aquaalert_member_since", new Date().toISOString());
      }
    }
    createReport.mutate(
      { data: { ...data, severity: data.severity as ReportInputSeverity, latitude: position[0], longitude: position[1], imageUrl: imageDataUrl } },
      {
          ...
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-3 flex-1">
              <motion.div animate={{ scale: step === s ? 1.1 : 1 }}
                className={cn("w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all flex-shrink-0",
        ...
        <Card className="bg-slate-900 border-slate-800 shadow-2xl">
          <CardHeader className="border-b border-slate-800 px-6 py-5">
            ...
            <CardDescription className="text-slate-500 text-sm">
              {step === 1 ? "Click anywhere on the map to drop a pin exactly where the leak is."
                         : "Provide details to help the dispatch team respond quickly."}
            </CardDescription>
          </CardHeader>
              ...
              {step === 1 ? (
                <motion.div key="step1" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-5">
                  ...
                  <div className="flex justify-between items-center">
                    {position
                      ? <span className="text-sm text-green-400 flex items-center gap-1.5 font-medium"><CheckCircle className="w-4 h-4" />Pinned ({position[0].toFixed(4)}, {position[1].toFixed(4)})</span>
                      : <span className="text-sm text-slate-500">Tap the map to place your pin</span>}
                    <Button disabled={!position} onClick={() => setStep(2)} className="bg-cyan-600 hover:bg-cyan-500 text-white px-6">Next →</Button>
                  </div>
                </motion.div>
              ) : (
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                          ...
                          <FormControl>
                            <div className="grid grid-cols-4 gap-2">
                              {(["low", "medium", "high", "critical"] as const).map((sev) => {
                                ...
                                return (
                                  <motion.label key={sev} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                                    className={cn("flex flex-col items-center justify-center p-3 rounded-xl border-2 cursor-pointer transition-all text-center",
                      ...
                      <FormField control={form.control} name="description" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea placeholder="Describe the leak and nearby landmarks to help the team locate it quickly…"
                              className="bg-slate-950 border-slate-800 text-slate-100 focus:border-cyan-600 rounded-xl min-h-[80px] resize-none"
                              {...field} />
                          </FormControl>
                      ...
                      <div className="space-y-2">
                        <label className="text-slate-300 text-sm font-medium flex items-center gap-2">
                          <Camera className="w-4 h-4 text-slate-500" />Photo Evidence <span className="text-slate-600 font-normal text-xs">(optional)</span>
                        </label>
                        {imageDataUrl ? (
                          <div className="relative rounded-xl overflow-hidden border border-slate-800 bg-slate-950">
                            <img src={imageDataUrl} alt="Preview" className="w-full h-40 object-cover" />
                            <button type="button" onClick={() => { setImageDataUrl(null); if (fileInputRef.current) fileInputRef.current.value = ""; }}
                              className="absolute top-2 right-2 w-7 h-7 bg-slate-900/90 rounded-full flex items-center justify-center border border-slate-700 hover:bg-red-950 hover:border-red-800 transition-colors">
                          ...
                          <div
                            ...
                            className="w-full h-24 border-2 border-dashed border-slate-800 hover:border-cyan-800 rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer transition-colors bg-slate-950/60 hover:bg-slate-900/60"
                          >
                            {imageLoading
                              ? <><div className="w-5 h-5 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" /><span className="text-xs text-slate-500">Processing…</span></>
                              : <><Image className="w-5 h-5 text-slate-600" /><span className="text-xs text-slate-500">Tap to attach a photo</span><span className="text-[10px] text-slate-700">Compresses automatically · max 10 MB</span></>}
                          </div>
                        )}
